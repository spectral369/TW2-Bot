...
public class Depozit {
	...
	Robot r= null;
	int numarRe = 0;
	public Depozit(WebDriver driver,WebDriverWait wait,Robot robot,int numarReincarcari) throws InterruptedException{
	WebElement elem  = null;
	this.wait = wait;	
	this.r = robot;
	boolean isBot=false;
	this.numarRe = numarReincarcari;
	
		
		Thread.sleep(40000);//40 sec test
		try {
	...
	//	elem= driver.findElement((By.xpath("//div[@id='interface-chat']/div/div[2]/div/div[10]/div[2]/div/div[10]")));
				elem= wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//div[@id='interface-chat']/div/div[2]/div/div[10]/div[2]/div/div[10]")));
			//elem.click();check somethign new
	...
	WebElement start = null;
	 if(dr.toUpperCase().contains("CHRO"))
	 {
		...
		start  = driver.findElement(By.cssSelector(".reward-corner .ng-binding"));
			...
			if(numarRe>0) {
				System.out.println("reincarcari1 !!! "+numarRe);
				WebElement rein =  driver.findElement(By.partialLinkText("articol"));
				 actions.moveToElement(rein).perform();
				 rein.click();
				 Thread.sleep(1000);
				 rein =  driver.findElement(By.partialLinkText("Folose"));
				 rein.click();
				 Thread.sleep(1000);
				 rein =  driver.findElement(By.partialLinkText("Articolul"));
				 rein.click();
		...
		//start =  driver.findElement(By.cssSelector(".reward-corner .ng-binding"));
			...
			System.out.println("reincarcari2 !!! "+numarRe);
			if(numarRe>0) {
				System.out.println("reincarcari2 !!! "+numarRe);
				WebElement rein =  driver.findElement(By.partialLinkText("articol"));
				 actions.moveToElement(rein).perform();
				 ...
				 numarRe--;
				 Thread.sleep(2000);
				 yyu =driver.findElements(By.xpath("//a[contains(text(),'Start')]")).size() > 0;
				 bottom =  driver.findElement(By.xpath("//a[contains(text(),'Start')]"));
